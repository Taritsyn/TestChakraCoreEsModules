namespace TestChakraCoreEsModules.JsRt
{
	...
	internal static class JsErrorHelpers
	{
					...
					case JsErrorCode.ScriptCompile:
						{
							...
							throw CreateScriptExceptionFromMetadata(errorCode, errorMetadata);
						}
		...
		/// <summary>
		/// Creates a new script exception from the error metadata
		/// </summary>
		/// <param name="metadata">JavaScript object representing the error metadata</param>
		/// <returns>Instance of the <see cref="JsScriptException"/> class</returns>
		public static JsScriptException CreateScriptExceptionFromMetadata(JsValue metadata)
		{
			JsValue namePropertyValue = metadata.GetProperty("name");
			string type = namePropertyValue.ValueType == JsValueType.String ?
				namePropertyValue.ToString() : string.Empty;
			JsErrorCode errorCode = type == "SyntaxError" ?
				JsErrorCode.ScriptCompile : JsErrorCode.ScriptException;

			return CreateScriptExceptionFromMetadata(errorCode, metadata);
		}
		...
		/// <param name="metadata">JavaScript object representing the error metadata</param>
		/// <returns>Instance of the <see cref="JsScriptException"/> class</returns>
		public static JsScriptException CreateScriptExceptionFromMetadata(JsErrorCode errorCode, JsValue metadata)
		{
			string message = errorCode == JsErrorCode.ScriptCompile ?
				"Compile error." : "Script threw an exception.";

			return new JsScriptException(errorCode, metadata, message);
		}
